    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl flex flex-col h-[90vh]">
        <div className="px-6 py-4 border-b border-neutral-200 bg-neutral-50 space-y-3">
          <DrilldownHeader
            title="Outbreak (Active)"
            subtitle="Filtered view"
            right={
              <ExportPdfButton
                filename="active-outbreaks"
                buildSpec={() => ({
                  title: "Outbreak (Active)",
                  orientation: "landscape",
                  template: "LANDSCAPE_TEMPLATE_V1",
                  subtitleLines: [`Filters Applied: Active outbreaks: ${activeOutbreaks.length}`],
                  sections: activeOutbreaks.map((outbreak) => ({
                    type: "table" as const,
                    title: outbreak.title || "Outbreak",
                    columns: ["Resident", "Status", "Symptom Onset"],
                    rows: (Object.values(store.outbreakCases) as OutbreakCase[])
                      .filter((c) => c.outbreakId === outbreak.id)
                      .map((caseItem) => {
