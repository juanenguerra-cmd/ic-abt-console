...
import {
  restoreFromPrevAsync,
  hardResetStorageAsync,
  runMigrations,
  packV3,
  MAX_STORAGE_CHARS,
} from "../../storage/engine";
...
const validateAndRepairDb = (
  db: unknown,
): { valid: boolean; error?: string; repairedDB?: UnifiedDB } => {
  ...
  if (!activeId || !facilityIds.includes(activeId)) {
    if (facilityIds.length === 1) {
      ...
      console.warn(
        "Repaired activeFacilityId by setting it to the only available facility.",
      );
    } else {
      return {
        valid: false,
        error: `Invalid or missing activeFacilityId: '${activeId}'.`,
      };
    }
  }
  ...
  if (!workingDB.data.facilityData[activeId]) {
    console.warn(
      `Missing facilityData for active facility '${activeId}'. Initializing empty store.`,
    );
...
export const SettingsConsole: React.FC = () => {
  ...
  const [isUnitRoomConfigModalOpen, setIsUnitRoomConfigModalOpen] =
    useState(false);
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem(
        `ltc_floor_tile_size_global:${activeFacilityId}`,
      );
  ...
  const handleExport = () => {
    const packed = packV3(db);
    const blob = new Blob([JSON.stringify(packed, null, 2)], {
    ...
    if (confirm("Are you sure? This will overwrite current data.")) {
      ...
      try {
        ...
        if (ok) {
          ...
          alertService.show("Previous version restored. App will reload.", {
            type: "success",
          });
  ...
  const handleFullReset = async () => {
    if (confirm("Are you absolutely sure? This will erase all data.")) {
      ...
      alertService.show(
        "All local data has been erased. The app will now reload.",
        { type: "success" },
      );
      setTimeout(() => window.location.reload(), 1000);
    }
  ...
  const restoreBackup = async (file: File) => {
    ...
    try {
      ...
      const liveResidents = Object.keys(
        repairedDB.data.facilityData[facId]?.residents?.byId || {},
      ).length;

      if (
        liveResidents <= 0 &&
        Object.keys(repairedDB.data.facilityData[facId]?.abts?.byId || {})
          .length <= 0
      ) {
        // Check a few stores to be sure. If a backup is truly empty, that's fine, but if it had data and now doesn't, it's an error.
        const originalResidentCount = Object.keys(
          normalized.data.facilityData[facId]?.residents?.byId || {},
        ).length;
        if (originalResidentCount > 0) {
          throw new Error(
            "Restore completed, but the live application state is unexpectedly empty. Aborting to prevent data loss.",
          );
        }
      }

      alertService.show(
        "Backup restored successfully! The application will now reload to apply all changes.",
        { type: "success" },
      );
      ...
      alertService.show(`${err instanceof Error ? err.message : String(err)}`, {
        type: "error",
      });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 p-4">
      ...
      />
      <UnitRoomConfigModal
        isOpen={isUnitRoomConfigModalOpen}
        onClose={() => setIsUnitRoomConfigModalOpen(false)}
      />
          ...
          <div className="flex flex-col sm:flex-row gap-8 items-start">
            <div className="flex-1 space-y-4">
              <p className="text-sm text-neutral-600">
                Adjust the size of room tiles on the Floor Layout view. This
                affects how many rooms fit on screen.
              </p>
              <div>
                <input
                  type="range"
            ...
            <div className="w-48 h-32 bg-neutral-100 rounded-md border border-neutral-200 flex items-center justify-center overflow-hidden">
              <div
                className="bg-white border-2 border-indigo-300 rounded shadow-sm flex items-center justify-center text-indigo-700 font-bold transition-all duration-200"
                style={{
                  width: previewTileW,
                  height: previewTileH,
                  fontSize: 10 + tileSize,
                }}
              >
                101
              </div>
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <div className="px-4 py-5 sm:px-6 border-b border-neutral-200 bg-neutral-50 flex items-center">
          <Building2 className="h-5 w-5 text-indigo-500 mr-2" />
          <h3 className="text-lg leading-6 font-medium text-neutral-900">
            Unit & Room Mapping Configuration
          </h3>
        </div>
        <div className="px-4 py-5 sm:p-6 space-y-4">
          <p className="text-sm text-neutral-600">
            Configure the available units for your facility and map specific
            room numbers to each unit.
          </p>
          <button
      ...
      {can("*") && (
        <div className="bg-white shadow rounded-lg overflow-hidden">
          ...
          <div className="px-4 py-5 sm:p-6 space-y-4">
            <p className="text-sm text-neutral-600">
              Change your current role to test different permission levels.
            </p>
            <div className="flex items-center gap-4">
              <select
                value={pendingRole || role}
                onChange={(e) => setPendingRole(e.target.value as UserRole)}
                className="border border-neutral-300 rounded-md p-2 text-sm focus:ring-indigo-500 focus:border-indigo-500"
              >
              ...
              <button
                onClick={() => {
                  if (pendingRole) {
                    setRole(pendingRole);
                    alertService.show(`Role changed to ${pendingRole}`, {
                      type: "success",
                    });
                    setPendingRole(null);
                  }
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        ...
        <div className="px-4 py-5 sm:p-6 space-y-4">
          <p className="text-sm text-neutral-600">
            Manage historical residents not currently on census.
          </p>
          <button
            ...
            <div className="sm:col-span-1">
              ...
              <dd className="mt-1 text-sm text-neutral-900">
                ...
                <div className="mt-1 text-xs text-neutral-500">
                  ...
                  {(MAX_STORAGE_CHARS / 1024).toFixed(1)} KB
                </div>
              </dd>
            </div>
            <div className="sm:col-span-1">
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden border border-red-100">
        ...
        <div className="px-4 py-5 sm:p-6">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            >
              <RefreshCw
                className={`-ml-1 mr-2 h-5 w-5 ${isRestoreBusy ? "animate-spin" : ""}`}
              />
          ...
          <div className="mt-6 p-4 border border-yellow-300 bg-yellow-50 rounded-md">
            ...
            <p className="text-xs text-yellow-700 mt-1">
              This will overwrite all current data. This action cannot be
              undone.
            </p>
