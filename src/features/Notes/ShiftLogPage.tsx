...
export const ShiftLogPage: React.FC = () => {
  ...
  const markLineListCleared = (entryId: string) => {
    updateDB(draft => {
      const fd = draft.data.facilityData[activeFacilityId];
      if (!fd.shiftLog?.[entryId]) return;
      fd.shiftLog[entryId].lineListClearedAtISO = new Date().toISOString();
    });
  };
  ...
  const filteredEntries = useMemo(() => {
    return entries
      .filter(e => {
        ...
        if (filterAutoOnly && !e.autoGenerated) return false;
        if (
          filterLineListNeeded &&
          (!e.autoGenerated ||
            !shouldSuggestLineList(e.sourceResidentHashtag ?? '') ||
            Boolean(e.lineListClearedAtISO))
        ) return false;
        if (searchText.trim()) {
  ...
  }, [entries, filterShift, filterUnit, filterTag, filterPriority, filterLast24h, filterAutoOnly, filterLineListNeeded, searchText, twentyFourHoursAgo]);
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] bg-neutral-100">
      ...
      <div className="bg-white border-b border-neutral-200 px-6 py-2 flex flex-wrap items-center gap-2 shrink-0">
        ...
        <button
          onClick={() => setFilterAutoOnly(v => !v)}
          className={`px-2.5 py-1 rounded-full text-xs font-medium border transition-colors ${filterAutoOnly ? 'bg-yellow-100 border-yellow-400 text-yellow-800' : 'bg-white border-neutral-300 text-neutral-600'}`}
        >
          ⚡ Auto-Generated
        </button>

        <button
          onClick={() => setFilterLineListNeeded(v => !v)}
          className={`px-2.5 py-1 rounded-full text-xs font-medium border transition-colors ${filterLineListNeeded ? 'bg-indigo-100 border-indigo-400 text-indigo-800' : 'bg-white border-neutral-300 text-neutral-600'}`}
        >
          📋 Line List Needed
        </button>
        ...
        {filteredEntries.length === 0 ? (
          ...
          filteredEntries.map(entry => (
            <div key={entry.id} className={`bg-white rounded-lg border p-4 shadow-sm ${entry.priority === 'Action Needed' ? 'border-red-200' : 'border-neutral-200'}`}>
              ...
              {/* Line List actions */}
              {entry.autoGenerated && shouldSuggestLineList(entry.sourceResidentHashtag ?? '') && (
                <div className="mt-2 flex items-center justify-end gap-2">
                  {entry.lineListClearedAtISO ? (
                    <span className="px-2 py-1 rounded-full text-[10px] font-semibold uppercase bg-emerald-50 text-emerald-700 border border-emerald-200">
                      Cleared
                    </span>
                  ) : (
                    <>
                      <button
                        onClick={() => navigate('/linelist-report', { state: { symptomClass: getHashtagSymptomClass(entry.sourceResidentHashtag ?? '') } })}
                        className="flex items-center gap-1 px-3 py-1 bg-indigo-600 text-white rounded-md text-xs font-medium hover:bg-indigo-700"
                      >
                        📋 Quick Line List
                      </button>
                      <button
                        onClick={() => markLineListCleared(entry.id)}
                        className="px-3 py-1 bg-white text-emerald-700 border border-emerald-300 rounded-md text-xs font-medium hover:bg-emerald-50"
                      >
                        Clear
                      </button>
                    </>
