...
export const commandHandlers = {
  ...
  restorePrevious: async () => {
    const restoredDb = await restoreFromPrevAsync();
    if (restoredDb) {
      restoredDb.updatedAt = new Date().toISOString();
      await saveDBAsync(restoredDb);
      const activeFacilityId = restoredDb.data.facilities.activeFacilityId;
      const store = restoredDb.data.facilityData[activeFacilityId];
      if (store) {
