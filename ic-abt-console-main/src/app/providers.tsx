...
interface DatabaseContextType {
  db: UnifiedDB;
  updateDB: (updater: (draft: UnifiedDB) => void, meta?: MutationMeta) => Promise<void>;
...
export function AppProviders({ children }: { children: ReactNode }) {
  ...
  const setDB = useCallback((newDb: UnifiedDB) => {
    ...
    // Synchronously update timestamp to prevent reconciliation from overwriting
    // this restored data if a window focus event fires immediately.
    ...
    const savePromise = saveQueue.current.then(async () => {
      try {
        await commandHandlers.saveDatabase(newDb, newDb.data.facilities.activeFacilityId, [], true); // Force full save
      } catch (err) {
        console.error("DB_SAVE_FAILURE", err);
        window.dispatchEvent(new CustomEvent("DB_SAVE_FAILURE", { detail: err }));
        const msg = err instanceof Error ? err.message : "Failed to save database";
        ...
        setDb(current => {
          if (current === newDb) {
            if (prevDb) setActiveFacilityId(prevDb.data.facilities.activeFacilityId);
